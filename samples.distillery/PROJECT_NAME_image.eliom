...
let () =
 Ew_dyn_upload.register
      crop_service
      (Ebapp.Session.connected_fun crop_handler);
  ...
  let update_pictures dname fname =
    ...
    let pics =
      Dom_html.document##body##querySelectorAll
        (Js.string (".eba_avatar"))
    in
    for i = 0 to pics##length - 1 do
      Js.Opt.iter (pics##item(i))
        (fun elt ->
           let img = Js.Unsafe.coerce elt in
           if img##src = oldpic
           then img##src <- newpic)
    done
  ...
  let _ =
    ...
    let default_picture =
      Dom_html.document##body##querySelector
        (Js.string ("div.eba_identity img.eba_avatar"))
    in
    let src =
...
{shared{
  type crop_param_t =
      (%%%MODULE_NAME%%%_user.t * string list * string * (int * int * int * int)
      ) deriving (Json)
}}

module type T = sig
  val start_crop_on_clicking_on : Dom_html.element Js.t -> %%%MODULE_NAME%%%_user.t -> unit
end
