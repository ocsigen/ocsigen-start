(* This demo illustrates two features that relate to smooth browsing through the
   browser history.

   The first feature is Eliom's DOM caching feature. By running
   [Eliom_client.onload Eliom_client.push_history_dom] one can push the DOM of
   the current page into Eliom's cache. Every page which is cached in this
   manner will be immediately served from the cache instead of being charged
   from the server or regenerated by the client. Also the scroll position is
   restored that the page had at the end of the last visit.

   The second feature is an animated page transition when clicking on links
   (page slides in from the right) or when clicking on the back button (page
   slides in from the left). This feature depends on a procedure to take
   screenshots of the current view. The demo relies on a cordova plugin,
   therefore the transition animation will only be seen on a cordova-generated
   app.
*)
