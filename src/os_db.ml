...
module User =
  struct
    ...
    let is_registered email =
      Lwt.catch (fun () -> let* _ = userid_of_email email
                            in Lwt.return_true)
        (function
                     ...
                     let params : string option list list =
                       [[Some
                           (((let open PGOCaml in string_of_string))
                              firstname)];
                       [Some
                          (((let open PGOCaml in string_of_string)) lastname)];
                       [PGOCaml_aux.Option.map
                          (let open PGOCaml in string_of_string) email];
                       [PGOCaml_aux.Option.map
                          (let open PGOCaml in string_of_string) password_o];
                       [PGOCaml_aux.Option.map
                          (let open PGOCaml in string_of_string) avatar];
                       [PGOCaml_aux.Option.map
                          (let open PGOCaml in string_of_string) language]] in
                     let split =
                       [`Text
                       ...
                       (let dbh = dbh in
                        let params : string option list list =
                          [[Some
                              (((let open PGOCaml in string_of_string)) email)];
                          [Some
                             (((let open PGOCaml in string_of_int64)) userid)]] in
                        let split =
                          [`Text
                        ...
                        PGOCaml.bind
                          ...
                          (fun () -> PGOCaml.execute_rev dbh ~name ~params ()))
                       (fun _rows -> PGOCaml.return ())
                    in remove_preregister0 dbh email
               | None -> Lwt.return_unit
              in Lwt.return userid)
    ...
    let remove_email_from_user ~userid  ~email  =
      let* b = is_main_email ~userid ~email
       in
      if b
                                           ...
                                           (String.concat "; "
                                              (List.map
                                                 (function
                                                  ...
                                                  | None -> "NULL") row))))) in
                              raise (PGOCaml.Error msg)) _rows))
            in Lwt.return (match l with | _::[] -> true | _ -> false))
    let exists number =
      Lwt.bind
        (without_transaction @@
           (fun dbh ->
