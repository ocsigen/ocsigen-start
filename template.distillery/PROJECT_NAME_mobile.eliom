...
let%client app_started = ref false
let%client initial_change_page = ref None

let%client change_page_gen action =
  if !app_started
...
let%client handle_initial_url () =
  ...
  | None ->
      Eliom.Client.change_page ~replace:true ~service:Os_services.main_service
        () ()
  | Some action -> action ()
