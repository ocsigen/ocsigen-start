...
let main_title capitalized_app_name =
  let title =
    Eliom_content.Html5.Id.create_global_elt
      (D.h1 [a ~service:Ol_services.main_service
  ...
  ignore {unit{
    Lwt_js_events.async (fun () ->
      Lwt_js_events.clicks (To_dom.of_div %b)
        (fun _ _ -> Eliom_client.change_page %Ol_services.logout_service () ()))
    }};
  b
...
let _ =
  ...
  current_pic := Js.Opt.case
    (Dom_html.document##body##querySelector
       (Js.string ("div.ol_identity img."^Ol_common0.cls_avatar)))
    (fun () -> Js.string "")
...
let update_pics newpic =
  ...
  let newpic =
    Js.string (Ol_common0.make_pic_string_uri ~absolute:true newpic) in
  current_pic := newpic;
  let pics =
    Dom_html.document##body##querySelectorAll
      (Js.string ("."^Ol_common0.cls_avatar))
  in
  for i = 0 to pics##length - 1 do
    let img = Js.Unsafe.coerce (Ol_misc.of_opt (pics##item(i))) in
              ...
              Js.Opt.case (files##item(0))
                ...
                (fun file ->
                  lwt newpic = Eliom_client.call_caml_service
                    ~service:%Ol_services.pic_service
                    () file
                  in
                  (* close form *)
                  me#unpress;
  ...
  ignore {unit{
    ...
    ignore (object (me)
      inherit [ Html5_types.div_content_fun ] Ew_buh.alert
        ...
        ~button:d
(*        ~parent_node:(Ol_misc.of_opt d##parentNode) *)
        ()
