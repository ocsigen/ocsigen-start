...
- [`Os_services`](./ocsigen-start.server/Os_services.md): Eliom services pre-defined by Ocsigen Start. These produce the default user interface of the template. See [the Eliom manual page on services](https://ocsigen.org/eliom/latest/server-services.html) for a general introduction to Eliom services.
- [`Os_handlers`](./ocsigen-start.server/Os_handlers.md): The handlers (functions) that implement the services in [`Os_services`](./ocsigen-start.server/Os_services.md). [The Eliom manual page on service handlers](https://ocsigen.org/eliom/latest/server-outputs.html) explains how to implement handlers.
