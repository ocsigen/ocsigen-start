...
let create' ?password ?avatar ~firstname ~lastname email =
  try_lwt
    lwt _ = Foobar_db.User.uid_of_email email in
    Lwt.fail Already_exists
  with No_such_resource ->
    lwt uid =
      Foobar_db.User.create ~firstname ~lastname ?password ?avatar email
    in
    lwt u = Foobar_db.User.user_of_uid uid in
    Lwt.return (create_user_from_db u)

(* Overwrite the function [update] of [Foobar_db.User] to reset the cache *)
let update ?password ?avatar ~firstname ~lastname uid =
  lwt () = Foobar_db.User.update ?password ?avatar ~firstname ~lastname uid in
  MCache.reset uid;
  Lwt.return ()
