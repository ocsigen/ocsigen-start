...
struct
  ...
  let get key =
    try_lwt
      lwt g = Foobar_db.Groups.group_of_name key in
...
let create ?description name =
  let group_of_name name =
    lwt g = Foobar_db.Groups.group_of_name name in
    Lwt.return (create_group_from_db g)
  in
  try_lwt group_of_name name with
  | No_such_resource ->
    lwt () = Foobar_db.Groups.create ?description name in
    try_lwt
      lwt g = group_of_name name in
      Lwt.return g
    with No_such_resource -> Lwt.fail No_such_group (* Should never happen *)

(** Overwrite the function [group_of_name] of [Foobar_db.User] and use
...
let add_user_in_group ~group =
  Foobar_db.Groups.add_user_in_group ~groupid:(group.id)
let remove_user_in_group ~group =
  Foobar_db.Groups.remove_user_in_group ~groupid:(group.id)
let in_group ~group =
  Foobar_db.Groups.in_group ~groupid:(group.id)

(** Returns all the groups of the database. *)
let all () =
  lwt groups = Foobar_db.Groups.all () in
